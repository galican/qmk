...
static const uint8_t rgb_index_table[]          = {LED_HOST_USB_INDEX, LED_HOST_BT1_INDEX, LED_HOST_BT2_INDEX, LED_HOST_BT3_INDEX, LED_HOST_2G4_INDEX};
static const uint8_t rgb_index_color_table[][3] = {
    LED_HOST_USB_COLOR, LED_HOST_BT1_COLOR, LED_HOST_BT2_COLOR, LED_HOST_BT3_COLOR, LED_HOST_2G4_COLOR,
};
...
#ifdef WL_BT_SW_PIN
    gpio_set_pin_input_high(WL_BT_SW_PIN);
    gpio_set_pin_input_high(WL_RF_SW_PIN);
#endif
...
static void bt_scan_mode(void) {
#ifdef WL_BT_SW_PIN
    if (!gpio_read_pin(WL_BT_SW_PIN) && gpio_read_pin(WL_RF_SW_PIN)) {
        if ((dev_info.devs == DEVS_USB) || (dev_info.devs == DEVS_2_4G)) {
            bt_switch_mode(dev_info.devs, dev_info.last_devs, false); // BT mode
        }
    } else if (!gpio_read_pin(WL_RF_SW_PIN) && gpio_read_pin(WL_BT_SW_PIN)) {
        if (dev_info.devs != DEVS_2_4G) {
            bt_switch_mode(dev_info.devs, DEVS_2_4G, false); // 2_4G mode
        }
    } else if (gpio_read_pin(WL_BT_SW_PIN) && gpio_read_pin(WL_RF_SW_PIN)) {
        if (dev_info.devs != DEVS_USB) {
