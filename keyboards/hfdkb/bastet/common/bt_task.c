    ...
    switch (keycode) {
        ...
        case SW_OS1: {
            if (get_highest_layer(default_layer_state) == 0) { // WIN_BASE
                ...
                single_blink_cnt   = 6;
                single_blink_color = (RGB){RGB_WHITE};
#ifdef LED_KEY_G_INDEX
                single_blink_index = LED_KEY_G_INDEX;
#endif
#ifdef LED_KEY_H_INDEX
                single_blink_index = LED_KEY_H_INDEX;
#endif
...
static void close_rgb(void) {
    ...
    /*************************************************************************************/
    if (sober) {
        if (kb_sleep_flag || (timer_elapsed32(key_press_time) >= ((2 * 60 - 10) * 1000))) {
            ...
            rgb_matrix_disable_noeeprom();
#ifdef WS2812_EN_PIN
            gpio_write_pin_high(WS2812_EN_PIN);
#endif
        }
    } else {
        if (!rgb_matrix_config.enable) {
...
void open_rgb(void) {
    ...
    if (!sober) {
        ...
        gpio_write_pin_low(WS2812_EN_PIN);
#endif
        if (bak_rgb_toggle) {
            extern bool low_vol_offed_sleep;
            kb_sleep_flag       = false;
...
uint8_t bt_indicator_rgb(uint8_t led_min, uint8_t led_max) {
    if (!dev_info.Charge_flag) {
        ...
        if (!gpio_read_pin(BT_CABLE_PIN)) {
            ...
            if (low_bat_level) {
                ...
                rgb_matrix_set_color(LED_PWR_INDEX, Low_power_bink ? 100 : 0, 0, 0);
            }
    ...
    if ((dev_info.devs != DEVS_USB) && gpio_read_pin(BT_CABLE_PIN)) {
        if (query_vol_flag) {
            rgb_matrix_set_color_all(0, 0, 0);

            uint8_t pvol                  = bts_info.bt_info.pvol;
            uint8_t led_count             = 0;
            uint8_t power_check_table[10] = LED_BAT_INDEX_TAB;
            ...
            for (uint8_t i = 0; i < led_count; i++) {
                rgb_matrix_set_color(power_check_table[i], color.r, color.g, color.b);
            }
        }
    }
