...
long_pressed_keys_t long_pressed_keys[] = {
  ...
  {.keycode = SW_OS2, .press_time = 0, .event_cb = long_pressed_keys_cb},
  {.keycode = SW_OS3, .press_time = 0, .event_cb = long_pressed_keys_cb},
...
#define SINGLE_HOLD_TIME 2000
uint8_t  single_hold_index;
uint32_t single_hold_time;
RGB      single_hold_color = {0};
    ...
    switch (keycode) {
        ...
        case SW_OS1: {
            if (get_highest_layer(default_layer_state) != 0) {
                ...
                single_hold_time  = timer_read32();
                single_hold_index = LED_OS_WIN;
                single_hold_color = (RGB){100, 100, 100};
            }
        } break;
        case SW_OS2: {
        ...
        case SW_OS3: {
            if (get_highest_layer(default_layer_state) != 6) {
                ...
                eeconfig_update_keymap(&keymap_config);
                single_hold_time  = timer_read32();
                single_hold_index = LED_OS_LINUX;
                single_hold_color = (RGB){100, 100, 100};
            }
        } break;
        case WL_2_4G: {
...
static void bt_used_pin_init(void) {
    ...
    gpio_set_pin_output_push_pull(LED_RED_PIN);
    gpio_set_pin_output_push_pull(LED_GREEN_PIN);
    gpio_set_pin_output_push_pull(LED_BLUE_PIN);

    gpio_write_pin_high(LED_RED_PIN);
    gpio_write_pin_high(LED_GREEN_PIN);
    gpio_write_pin_high(LED_BLUE_PIN);
#endif
}
...
static void bt_scan_mode(void) {
    ...
    } else if (gpio_read_pin(RF_MODE_SW_PIN) && !gpio_read_pin(BT_MODE_SW_PIN)) {
        if ((dev_info.devs == DEVS_USB) || (dev_info.devs == DEVS_2_4G)) bt_switch_mode(dev_info.devs, dev_info.last_devs, false); // BT mode
    } else if (gpio_read_pin(BT_MODE_SW_PIN) && gpio_read_pin(RF_MODE_SW_PIN)) {
...
void led_config_all(void) {
    if (!led_inited) {
        ...
        // gpio_set_pin_output_push_pull(RGB_MATRIX_SDB_PIN);
        // gpio_write_pin_high(RGB_MATRIX_SDB_PIN);
#endif
        if (((dev_info.devs != DEVS_USB) && (bts_info.bt_info.paired)) || (USB_DRIVER.state == USB_ACTIVE)) {
            gpio_write_pin(LED_RED_PIN, !host_keyboard_led_state().caps_lock);
            gpio_write_pin(LED_GREEN_PIN, !host_keyboard_led_state().caps_lock);
            gpio_write_pin(LED_BLUE_PIN, !host_keyboard_led_state().caps_lock);
        }
    }
}
...
#ifdef RGB_MATRIX_SDB_PIN
        // gpio_set_pin_output_push_pull(RGB_MATRIX_SDB_PIN);
        // gpio_write_pin_low(RGB_MATRIX_SDB_PIN);
#endif
...
uint8_t bt_indicator_rgb(uint8_t led_min, uint8_t led_max) {
    ...
    /*************************************************************************************/
    if (timer_elapsed32(single_hold_time) <= SINGLE_HOLD_TIME) {
        rgb_matrix_set_color(single_hold_index, single_hold_color.r, single_hold_color.g, single_hold_color.b);
    }
    /*************************************************************************************/
    if (!gpio_read_pin(BT_CABLE_PIN)) {
        if (!gpio_read_pin(BT_CHARGE_PIN)) {
            ...
            if (timer_elapsed32(charg_full_time) > 2000) {
                if (timer_elapsed32(charg_full_time) < 5000) {
                    ...
                    // }
                    rgb_matrix_set_color(LED_PWR_ALRM, 0, 100, 0);
                }
            }
        }
        ...
        if (low_battery_vol) {
            ...
            if (Low_power_bink) {
                ...
                //     rgb_matrix_set_color(i, 0, 0, 0);
                // }
                rgb_matrix_set_color(LED_PWR_ALRM, 0, 0, 0);
            }
        }
    ...
    if (dev_info.devs != DEVS_USB) {
        ...
        if (!bt_init_time && !kb_sleep_flag && timer_elapsed32(query_vol_time) >= 4000) {
            ...
            if ((uart_data_read[0] == 0xA7) && (uart_data_read[2] == ((uart_data_read[0] + uart_data_read[1]) & 0xFF))) {
                if ((uart_data_read[1] <= 10) && gpio_read_pin(BT_CABLE_PIN)) {
                    low_battery_vol = true;
                }
                if ((uart_data_read[1] < 1) && gpio_read_pin(BT_CABLE_PIN)) {
