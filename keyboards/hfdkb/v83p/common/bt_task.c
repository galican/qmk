...
static void bt_used_pin_init(void) {
    ...
    gpio_set_pin_input(BT_MODE_SW_PIN);
    gpio_set_pin_input(RF_MODE_SW_PIN);
#endif
...
static const uint8_t rgb_index_table[]          = {0, 15, 16, 17, 18};
static const uint8_t rgb_index_color_table[][3] = {
    {RGB_BLACK}, {RGB_BLUE}, {RGB_BLUE}, {RGB_BLUE}, {RGB_GREEN},
};
...
static void close_rgb(void) {
    ...
    if (sober) {
        if (kb_sleep_flag || (timer_elapsed32(key_press_time) >= (30 * 60 * 1000))) { // 3 minutes
...
#    if defined(BT_CABLE_PIN) && defined(BT_CHARGE_PIN)
    ...
    if (gpio_read_pin(BT_CABLE_PIN)) {
        if (!gpio_read_pin(BT_CHARGE_PIN)) {
            // 正在充电
            if (timer_elapsed32(charging_time) >= 2000) rgb_matrix_set_color(75, RGB_RED);
        } else {
            // 充满
...
#    endif
    if (dev_info.devs != DEVS_USB) {
        if (query_vol_flag) {
            for (uint8_t i = 15; i < 15 + (bts_info.bt_info.pvol / 10); i++) {
